...
class MainGameScene: SKScene, SKPhysicsContactDelegate {
    ...
    private lazy var backToInitial: Button = {
        let bt = Button(size: CGSize(width: 50, height: 50), normalColor: .red, highlightedColor: .white)
    ...
    private func failureEffect(_ duration: TimeInterval, lastShape: ShapeType) {
        ...
        gradientNode.run(.textureActions(lastShape, duration: duration/4, availableTextureShape: ShapeType.allCases))
    }

    private func successEffect(_ duration: TimeInterval, shapeType: ShapeType) {
        gradientNode.configureTexture(AppCache.instance.gradient(shape: shapeType),
