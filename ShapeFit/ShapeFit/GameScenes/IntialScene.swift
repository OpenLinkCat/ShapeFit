...
class InitialScene: SKScene {
    ...
    init(score: Score?) {
        ...
        private func removeUIandPresentScene(_ scene:SKScene) {
            func present() {
                AppDelegate.mainGameViewController.mainGameView.presentScene(scene, transition: AppDefines.Transition.toGame)
            }
        ...
        private lazy var playButton: Button = {
            ...
            bt.addClosure(.touchUpInside, target: self, closure: { (currentScene, sender) -> () in
                let scene = currentScene.mainGameScene ?? MainGameScene()
        ...
        private lazy var leaderboardButton: Button = {
            ...
            bt.addClosure(.touchUpInside, target: self, closure: { (target, sender) -> () in
                let sc = target.score
                GCHelper.sharedInstance.showGameCenter(AppDelegate.mainGameViewController, viewState: .leaderboards)
            })
            return bt
        }()
        ...
        private lazy var shareButton: Button = {
            ...
            bt.addClosure(.touchUpInside, target: self, closure: { (currentScene, sender) -> () in
                ...
                let shareImageScene = ShareScene(size: shareImageSize, score: currentScene.score)
                ...
                let gameView = AppDelegate.mainGameViewController.mainGameView
        ...
        private lazy var rateButton: Button = {
            ...
            bt.addClosure(.touchUpInside, target: self, closure: { (target, sender) -> () in
                ...
                AppDelegate.mainGameViewController.present(alert, animated: true, completion: nil)
            })
            return bt
        }()
