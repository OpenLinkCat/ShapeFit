...
open class StackNode: SKSpriteNode {
    ...
    public init(length: CGFloat, fillMode: FillMode) {
        ...
        super.init(texture: nil, color: SKColor.clear, size: fillMode.size(length))
    }
        ...
        case .vertical:
            if automaticSpacing {
                ...
                for node in subNodes {
                    ...
                    node.position.y = -(accumulatedLength + f.size.height/2)
...
public extension SKNode {
    func removeNodeFromStack(_ withRefresh: Bool = true) {
        if let stack = self.parent as? StackNode {
            stack.remove(node: self, reload: withRefresh)
        } else {
            let message = "SKUtils ERROR: Node is not in a StackNode"
