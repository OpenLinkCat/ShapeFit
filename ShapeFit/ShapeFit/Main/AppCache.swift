...
class AppCache {
    ...
    func initializeInitialScreenBackgroundTexture(screenSize: CGSize) {
        if initialScreenBackgroundTexture == nil {
            initialScreenBackgroundTexture = SKTexture(radialGradient: CGSize(width: screenSize.width, height: screenSize.height/2), colors: [.purple, .darkPurple])
        }
    }
    ...
    func initializeGameTextures(with size: CGSize) {
        ...
        if gradients == nil {
            gradients = [.circle: SKTexture(radialGradient: adjustedSize, colors: [.green, .darkGreen]),
                         .triangle: SKTexture(radialGradient: adjustedSize, colors: [.purple, .darkPurple]),
                         .square: SKTexture(radialGradient: adjustedSize, colors: [.orange, .darkOrange]),
                         .pentagon: SKTexture(radialGradient: adjustedSize, colors: [.red, .darkRed])]
        }
